...
// Strict No-Custody: Blocks private keys, seed phrases, and credentials

// Common BIP39 English wordlist (first 50 words for heuristic detection)
const BIP39_WORDS = new Set([
  ...
  "brilliant", "bring", "brisk", "broccoli", "broken", "bronze", "broom", "brother",
  ...
  "holy", "home", "honey", "hood", "hope", "horn", "horror", "horse",
  ...
  "insane", "insect", "inside", "insight", "insist", "inspire", "install", "intact",
  ...
  "live", "lizard", "load", "loan", "lobster", "local", "location", "lock",
  "locust", "lodge", "log", "lonely", "long", "loop", "lottery", "loud",
  ...
  "marriage", "marsh", "master", "match", "material", "math", "matrix", "matter",
  "maximum", "meadow", "mean", "measure", "meat", "mechanic", "medal", "media",
  "melody", "melt", "member", "memory", "mention", "menu", "mercy", "merge",
  "merit", "merry", "mesh", "message", "metal", "method", "middle", "midnight",
  "milk", "million", "mimic", "mind", "minimum", "minister", "minor", "minute",
  "miracle", "mirror", "misery", "miss", "mistake", "mix", "mixed", "mixture",
  "mobile", "model", "modify", "mom", "moment", "monitor", "monkey", "month",
  ...
  "shave", "sheep", "sheet", "shelf", "shell", "shelter", "shield", "shift",
  ...
  "topic", "torch", "tornado", "tortoise", "toss", "total", "tourist", "toward",
...
export interface SecretDetectionResult {
  detected: boolean;
  type: "seed_phrase" | "private_key" | "base58_key" | "none";
  message: string;
  matchedText?: string;
}

/**
 * Detect if input contains potential secrets
 */
export function detectSecret(input: string): SecretDetectionResult {
  if (!input || input.trim().length === 0) {
    return { detected: false, type: "none", message: "" };
  }

  // Check for seed phrases (12-24 space-separated words)
  const seedPhraseResult = detectSeedPhrase(input);
  if (seedPhraseResult.detected) {
    return seedPhraseResult;
  }

  // Check for hex private keys (64+ hex characters)
  const hexKeyResult = detectHexPrivateKey(input);
  if (hexKeyResult.detected) {
    return hexKeyResult;
  }

  // Check for base58 encoded keys
  const base58Result = detectBase58Key(input);
  if (base58Result.detected) {
    return base58Result;
  }
 ...
 */
function detectSeedPhrase(input: string): SecretDetectionResult {
  // Normalize input
  ...
  // BIP39 seed phrases are exactly 12, 15, 18, 21, or 24 words
  const validLengths = [12, 15, 18, 21, 24];
  if (!validLengths.includes(words.length)) {
    return { detected: false, type: "none", message: "" };
  }

  // Check if most words are in BIP39 wordlist
  const bip39Matches = words.filter((word) => BIP39_WORDS.has(word)).length;
  const matchRatio = bip39Matches / words.length;

  // If >85% match BIP39 words, likely a seed phrase
  if (matchRatio > 0.85) {
    return {
      detected: true,
      type: "seed_phrase",
      message: "Potential seed phrase detected. Never store seed phrases in DAIO. Use hints only.",
      matchedText: words.slice(0, 3).join(" ") + "...",
    };
  }
...
/**
 * Detect hex-encoded private keys
 */
function detectHexPrivateKey(input: string): SecretDetectionResult {
  // Remove whitespace and common separators
  const cleaned = input.replace(/[\s\-_:,]/g, "");

  // Check for 64+ hex characters (typical private key length)
  ...
  if (hexPattern.test(cleaned)) {
    return {
      detected: true,
      type: "private_key",
      message: "Potential private key detected (hex format). Never store private keys in DAIO. Use hints only.",
      matchedText: cleaned.substring(0, 16) + "...",
    };
  }

  // Check for 0x prefixed hex
  const hexPrefixPattern = /^0x[0-9a-fA-F]{64}$/;
  if (hexPrefixPattern.test(cleaned)) {
    return {
      detected: true,
      type: "private_key",
      message: "Potential private key detected (0x-prefixed hex). Never store private keys in DAIO. Use hints only.",
      matchedText: cleaned.substring(0, 18) + "...",
    };
  }
...
/**
 * Detect base58 encoded keys (common in Bitcoin, Solana)
 */
function detectBase58Key(input: string): SecretDetectionResult {
  // Remove whitespace
  const cleaned = input.replace(/\s/g, "");

  // Base58 pattern: alphanumeric excluding 0, O, I, l
  // Typical key lengths: 44-58 chars (Solana), 51-52 chars (Bitcoin WIF)
  const base58Pattern = /^[1-9A-HJ-NP-Za-km-z]{44,58}$/;

  if (base58Pattern.test(cleaned)) {
    // Additional heuristic: check for common key prefixes
    const commonPrefixes = ["5", "K", "L", "xprv", "xpub", "s"];
    const hasCommonPrefix = commonPrefixes.some((prefix) =>
      cleaned.startsWith(prefix)
    );

    // High entropy check: count unique characters
    const uniqueChars = new Set(cleaned).size;
    const entropy = uniqueChars / cleaned.length;

    // If high entropy and matches pattern, likely a key
    if (entropy > 0.7 || hasCommonPrefix) {
      return {
        detected: true,
        type: "base58_key",
        message: "Potential base58-encoded key detected. Never store private keys in DAIO. Use hints only.",
        matchedText: cleaned.substring(0, 12) + "...",
      };
    }
  }
...
/**
 * Validate that input is safe (no secrets)
 */
export function validateSafeInput(
  input: string,
  fieldName: string
): { valid: boolean; error?: string } {
  const detection = detectSecret(input);

  if (detection.detected) {
    return {
      valid: false,
      error: `${fieldName}: ${detection.message}`,
    };
  }
